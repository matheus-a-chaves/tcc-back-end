...
public interface PartidaRepository extends JpaRepository<Partida, Long> {
	...
	@Query(value = "SELECT p.* FROM partida p " +
				   "  LEFT JOIN amistoso a ON p.amistoso_id = a.id " +
				   "  LEFT JOIN dados_partida dp on p.id = dp.partida_id " +
	...
	@Query(value = "SELECT p.* FROM partida p " + 
				   " INNER JOIN dados_partida dp ON p.id = dp.partida_id " +
				   "  JOIN Amistoso a ON a.id = p.amistoso_id " + 
				   " WHERE date(p.data_partida) = :data " + 
				   "   AND dp.equipe_id = :idEquipe " +
				   "   AND a.status = 'CONFIRMADO'", nativeQuery = true)
